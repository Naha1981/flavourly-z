...
Task ID: 7-a
Agent: full-stack-developer
Task: Build landing page + 3-step onboarding wizard

Work Log:
- Read /home/z/my-project/worklog.md for full project context (Flavourly OS SPA, Next.js 16 + Prisma/Postgres + Tailwind v4, brand system in globals.css: --brand #FF6B00, --brand-light, --brand-dark, semantic tokens).
- Inspected existing patterns: src/components/flavourly/primitives.tsx (StatCard, StatusBadge, SectionHeading, EmptyState), src/components/flavourly/overlays/auth-overlay.tsx (overlay styling + brand button patterns), src/components/flavourly/views/dashboard-view.tsx (QRCodeSVG usage + downloadPoster pattern), src/lib/flavourly.ts (INDUSTRY_CURRENCY/LABELS/EMOJI, waMeUrl, formatPhone, toSlug), src/lib/store.ts (Zustand store — no onboarding slot yet, the parent will wire that), src/app/api/tenant/route.ts + whatsapp/connect + whatsapp/status route contracts, src/lib/validation.ts (tenantPatchSchema — onboardingCompleted NOT in allowed list, so PATCH silently strips it; route handler also has its own allowlist filter; per task rules I must NOT modify API routes, so the PATCH still returns 200 and the parent proceeds to onCompleted()).
- Created src/components/flavourly/landing-view.tsx — named export LandingView, "use client", props {onSignup, onLogin}. Full-screen (min-h-screen flex flex-col, footer mt-auto). 7 sections per spec:
  1. Hero: gradient from-brand to-amber-500, white text, full viewport (min-h-screen), logo 💡 + "Flavourly OS" wordmark, headline "Fill your empty chairs. Turn walk-ins into regulars." (text-4xl sm:text-5xl font-black), subtext about WhatsApp loyalty + SA context, two CTAs (Start Free Trial → onSignup, white bg; Log In onLogin, outline white border), "14-day free trial · No credit card" microcopy, industry badges row (6 chips using INDUSTRY_EMOJI + INDUSTRY_LABELS), bouncing scroll hint at bottom.
  2. The Problem (bg-white): "The Empty Chair Problem" — 3 cards (🪑 Empty seats earn R0, 📝 Paper punch cards die, 📱 Apps don't get installed).
  3. How It Works (bg-brand-light): 3 steps with circled number emojis (1️⃣ Print your QR / 2️⃣ They text JOIN / 3️⃣ You send promos), each with a Lucide icon chip (QrCode, MessageCircle, Megaphone).
  4. Industry-specific: 6 cards generated from INDUSTRY maps (restaurant→Points, cafe→Stamps, carwash→Washes, salon→Visits, barber→Cuts, retail→Points) — emoji tile + label + "Customers earn {currency} on every visit".
  5. Pricing teaser: Starter R299/mo 🌱 (5 features) + Growth R499/mo 🚀 (5 features, highlighted with brand border + "Most popular" badge).
  6. Final CTA: gradient from-brand to-amber-500, "Ready to fill your empty chairs?" + Start Free Trial → onSignup.
  7. Footer (bg-gray-900): "🇿🇦 Built in South Africa · POPIA compliant" + © year.
- Created src/components/flavourly/overlays/onboarding-overlay.tsx — named export OnboardingOverlay, "use client", props {tenant, onCompleted}. Full-screen fixed inset-0 z-[100] bg-background overflow-y-auto. Sticky top bar with Progress (value=(currentStep/3)*100) + "Step X of 3" + step label + Close X top-right (toast.warning with "Finish setup to unlock your dashboard" — does NOT actually close, preserves wizard).
  • Step 1 (BusinessProfile): Business Name (required, autoFocus), Industry Select (shadcn Select with emoji + label, INDUSTRY_LABELS), Currency Name Input (auto-syncs via setIndustry wrapper that also calls setCurrencyName(INDUSTRY_CURRENCY[v]) — but stays editable), Location/Address Input (optional), Brand Color (native <input type="color"> + 6 preset swatches: Flavourly Orange #FF6B00, Tomato Red #DC2626, Forest Green #16A34A, Amber #D97706, Rose Pink #DB2777, Teal #0D9488 — NO blue/indigo per design rules). Save & Continue → PATCH /api/tenant {name, industry, currencyName, brandColor, locationLabel} → toast.success("✅ Business profile saved!") → setCurrentStep(2). Validates name non-empty (toast.error if blank).
  • Step 2 (ConnectWhatsApp): on mount, if tenant.whatsappPhone already set, status="connected"; else GET /api/whatsapp/status → if connected show green card (CheckCircle2 + formatPhone(phone)), else status="disconnected" → "Link WhatsApp" button (#25D366 WhatsApp green). On link: POST /api/whatsapp/connect → if alreadyConnected, flip to connected; else show qrBase64 in <img> (data:image/png;base64 URL) + 4-step instructions (Open WhatsApp → Settings → Linked Devices → Link a Device → Scan). Poll GET /api/whatsapp/status every 4s (clears on connect); after autoConnectAfterMs (8000ms) POST /api/whatsapp/status to flip to connected (simulating the 'open' event). All intervals/timeouts cleaned up on unmount via refs. Back button + "Skip for now" link (sets step 3) + Continue button (sets step 3).
  • Step 3 (LoyaltyQr): QRCodeSVG value=waMeUrl(phoneForQr, "JOIN") where phoneForQr = waPhone ?? tenant.whatsappPhone ?? "27835550001" (demo fallback). size=240, fgColor=brandColor, bgColor=#ffffff, level="M". Below: Download PDF Poster button (opens new window with A4 print-ready HTML using qrserver.com API for the QR — same pattern as dashboard-view's downloadPoster, brand-colored, with business name + currency name + location + Flavourly OS footer + auto window.print()), Copy Link button (navigator.clipboard.writeText(joinLink) + toast.success), "Test the link yourself" anchor. "Go to Dashboard →" → PATCH /api/tenant {onboardingCompleted:true} (best-effort, .catch swallow — API silently strips this key per validation allowlist, but call succeeds with 200) → toast.success("🎉 Welcome to Flavourly!") → onCompleted(). Back button.
- All buttons use min-h-[44px] or min-h-[48px] for touch targets. Mobile-first responsive (grid-cols-1 → sm:grid-cols-2 → md:grid-cols-3). Inter font + font-black for headings. Emojis in every section + button + toast. SA context throughout (Rands, 🇿🇦, Braamfontein example).
- Used only the allowed imports: shadcn ui (button, input, label, card, select, progress), primitives (none actually needed — kept it lightweight), @/lib/flavourly helpers (INDUSTRY_*, waMeUrl, formatPhone), sonner toast, lucide-react icons, qrcode.react QRCodeSVG. No useFlavourly store import needed (props-driven). No next/auth, no fetch to other API routes.
- Lint: initial run flagged 1 warning (unused eslint-disable directive on the <img> for QR — the @next/next/no-img-element rule wasn't firing, so the disable was redundant). Removed the directive. Re-ran `bun run lint` — exit 0, 0 errors, 0 warnings across the whole project.
- TypeScript: `bunx tsc --noEmit` filtered for my two filenames — 0 errors in my files.
- Dev server log: shows "✓ Compiled in 207ms" after the file edits — new files compiled cleanly, no errors.

Stage Summary:
- Files created (exactly 2):
  - /home/z/my-project/src/components/flavourly/landing-view.tsx — public landing page (LandingView, "use client", props {onSignup, onLogin}). 7 sections: Hero (full-viewport gradient), The Problem, How It Works, Industry-specific, Pricing, Final CTA, Footer. Brand-styled, mobile-first, emojis, SA context.
  - /home/z/my-project/src/components/flavourly/overlays/onboarding-overlay.tsx — 3-step onboarding wizard (OnboardingOverlay, "use client", props {tenant, onCompleted}). Sticky Progress bar at top, Close X (toast warning), Back buttons on steps 2-3. Step 1: Business Profile (name/industry/currency/location/brandColor) → PATCH /api/tenant. Step 2: WhatsApp connect (poll status, show QR img, 4s polling + 8s auto-connect via POST /api/whatsapp/status, Skip option). Step 3: Loyalty QR (QRCodeSVG brand-colored size 240, Download PDF Poster, Copy Link, Go to Dashboard → PATCH /api/tenant {onboardingCompleted:true} → toast → onCompleted()).
- Key decisions:
  • Currency auto-sync: setIndustry wrapper updates both `industry` and `currencyName` state in one shot, but currencyName stays editable via its own Input — matches "auto-syncs on industry change, editable".
  • WhatsApp connect flow uses refs for poll interval + autoConnect timeout so they survive re-renders and are cleaned up on unmount (no leaks if user navigates away mid-scan).
  • Phone for Step 3 QR falls back to demo number "27835550001" if WhatsApp wasn't connected (matches dashboard-view.tsx's pattern) so the QR is never empty.
  • PATCH {onboardingCompleted:true} is best-effort — the existing API route + zod schema silently strip that key (not in allowed list), but the call still returns 200 and onCompleted() fires to advance to the dashboard. Per task rules I did NOT modify the API route/schema. The lead can add `onboardingCompleted` to the allowlist later if persistence is required.
  • Close X does NOT actually close the overlay — just shows a warning toast. The wizard is a gate to the dashboard; closing without a confirm modal would leave the user in an indeterminate state. If the lead wants a real close, they can add a confirmation dialog later.
